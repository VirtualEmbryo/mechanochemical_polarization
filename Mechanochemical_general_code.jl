...
function run(χ,λ⁻²,η,T,Δt,part) 
 ...
 # Lets write down some parameters in a txt just in case
  ...
  print(simulation*"len=$λ/"*"\n")
  pVTU="./VTU/"*simulation*"len=$λ/"
  mkpath(pVTU)
  pPNG="./PNG/"*simulation*"len=$λ/"
  mkpath(pPNG)
  mkpath(pPNG*"rac_time/") 
  mkpath(pPNG*"RhoA_time/") 
  mkpath(pPNG*"rho_time/") 
  mkpath(pPNG*"rho0_time/") 
  ...
  open(pPNG*"parameters.txt", "w") do f
    write(f, "L = $L\n")
    write(f, "Δx = $Δx\n") 
    write(f, "χ = $χ\n") 
    write(f, "k = $k\n")
  ...
  α₀o(x)= 10*exp(-0.5*(x[1])^2/((L/4)^2)) #higher prefactor for permanent polarization
  β₀o(x)= 2.25*exp(-0.5*(x[1]-L)^2/((L/4)^2))#2.25*β₀*exp(-0.5*(x[1]-L)^2/((L/4)^2))
  ...
  #SOLVING X AT t=0
  aₓ_0(x,w) = aₓ(0,x,w) #MILLORABLE LI HAURIA DE PASSAR EL PERFIL CORRECTE A TEMPS ZERO
  b_0(w) =  bₓ(0,0,w)
  op_x = AffineFEOperator(aₓ_0,b_0,X,WD0)
  uh_x = solve(op_x)
  uh_x_old =  uh_x

  #SOLVE ρ_b AT t=0 DONAT UNA VELOCITAT INICIAL ZERO 
  ...
  uh_v=zero(V) #encomptes de solve puc fer aixo per a comencar amb condicions inicials zero a V
  ...
  ten[end] = tension[end]
  ten[ten .< 0.0] .= 0.0 #removing negative values is unnecesary thanks to the threshold function nowadays
 ...
 #give steady state as initial conditions for rac and rho 
  ...
  for ti in t₀:Δt:(T-Δt)
    ...
    vCTErac = vCTE*threshold(rac1,rac0,racth)
    ...
    # Actualitzem v per la resolucio de rho i x
    ...
    #actualitem x_old per a la derivada temporal
    uh_x_old =  uh_x
    writevtk(Ω,pVTU*"VTU$i",cellfields=["x"=>uh_x,"v"=>uh_v,"rho"=>uh_ρ,"rho0"=>uh_ρ0])

    plotting("ρ_b",po,pPNG*"rho_time/","$i")
    plotting("rac",pa,pPNG*"rac_time/","$i")
    plotting("rac",paa,pPNG*"RhoA_time/","$i")
    plotting("ρ_u",poo,pPNG*"rho0_time/","$i")
  ...
  plots_run(nΔt,Δx,vt,xt,tensiont,ρt,ρ0t,ract,rhot,Mt,λ⁻²,pPNG,α, β, dᵃ, dᵇ,αt,βt)
...
σₐ₀ =  0#100        # [pN /um^2] maximum active "pressure"
...
#bar λ = sqrt( η L /(χ*L*L*M0) )

rac0=0.01#if i wanna make a switch for protrusion
vCTE=0#-0.15
ten0=10.0 #denominator for protrusion dependence on tension IF It's small there no protrusion and no tension created by it
tenth=5.0
...
#output folder name
const simulation= "Opto_inhib/Studying D/mca_rac2 decay T=$T ten0=$ten0 db=$dᵇ da=$dᵃ sig0=$sig0 rth=$ρth/F2B D rho0=$rho0 mTorc=$mTorc D=$D a0=$α₀ b0=$β₀ te=$τₑ ta=$τₐ M0=$M0 deltat=$Δt vCTE=$vCTE tenth=$tenth lb=$λᵇ/"
